from config import *
from app import Individual, MicroService
...
def threshold_distance(ind: Individual):
    thr_dist = 0.0
    for ms in ind.ms:
        thr_dist += abs(ind.cont[ms.cont_list[0]].res - MicroService.THRESHOLD[ms.id])
    return thr_dist
...
def _service_failure(ind: Individual, ms_id: int):
    ...
    for i in pm_allocated:
        pm = ind.pm[i]
        cnt = pm.ms_cnt[ms_id]
        srv_fail *= (pm.FAILURE + MicroService.FAILURE[ms_id] ** cnt)
    return srv_fail
...
def _service_mean_distance(ind: Individual, ms_id: int):
    ...
    for ms2_id in MicroService.CONSUMER[ms_id]:
